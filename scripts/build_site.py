...
def main() -> None:
    ...
    write_json(SITE / "api" / "v1" / "records.json", {"total": len(source.records), "data_updated_at": source.updated_at, "data": [r.model_dump() for r in source.records]})
    write_json(SITE / "api" / "v1" / "summary.json", {"data_updated_at": source.updated_at, "data": [r.model_dump() for r in summary(source.records)]})
